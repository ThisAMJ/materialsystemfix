...
void *Interface::GetPtr(const char *filename, const char *interfaceSymbol) {
	...
	if (!handle) {
		console->DevWarning("MSF: Failed to open module %s!\n", filename);
		return nullptr;
	}
	...
	if (!CreateInterface) {
		console->DevWarning("MSF: Failed to find symbol CreateInterface for %s!\n", filename);
		return nullptr;
	}
	...
	if (ret) {
		auto CreateInterfaceInternal = Memory::Read((uintptr_t)CreateInterface + 5);

#ifdef _WIN32
		auto s_pInterfaceRegs = Memory::DerefDeref<InterfaceReg *>(CreateInterfaceInternal + 6);
#else
		auto s_pInterfaceRegs = Memory::DerefDeref<InterfaceReg *>(CreateInterfaceInternal + 11);
#endif

		if (!CreateInterfaceInternal || !s_pInterfaceRegs) {
			console->DevWarning("MSF: Failed to find interface with symbol %s in %s! (old method failed)\n", interfaceSymbol, filename);
			return nullptr;
		}
		...
		for (auto &current = s_pInterfaceRegs; current; current = current->m_pNext) {
			if (!std::strncmp(current->m_pName, interfaceSymbol, std::strlen(interfaceSymbol))) {
            	...
            	// console->DevMsg("MSF: Found interface %s at %p in %s!\n", current->m_pName, fn, filename);
				break;
			}
		}
		...
		if (!fn) {
			...
			console->DevWarning("MSF: Failed to find interface with symbol %s in %s!\n", interfaceSymbol, filename);
			return nullptr;
		}
	} else {
		// console->DevMsg("MSF: Found interface %s (the easy way) at %p in %s!\n", interfaceSymbol, fn, filename);
	}
	return fn;
}
